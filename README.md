...
- Some pages currently fetch from `https://www.pict.live/api/...` directly (not from the local server). If you’re developing locally and want local data/APIs, update those fetch URLs to use your local base URL (or refactor to an env like `NEXT_PUBLIC_BASE_URL`).
...
- **CORS**: `app/middleware.js` only whitelists the production origin (`www.pict.live`). Local dev is fine for same-origin requests, but cross-origin testing may require adjusting `allowedOrigins`.
