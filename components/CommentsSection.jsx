...
const TYPE_META = {
  doubt: { label: "Doubt", badgeClass: "bg-amber-50 text-amber-700 border-amber-200" },
  tip: { label: "Tip", badgeClass: "bg-emerald-50 text-emerald-700 border-emerald-200" },
  experience: { label: "My experience", badgeClass: "bg-violet-50 text-violet-700 border-violet-200" },
...
export default function CommentsSection({ experienceId, companyName, articleAuthorName }) {
  ...
  const renderComment = (item, level = 0) => {
    ...
    return (
      <div key={item.id} className={cn("rounded-2xl border border-slate-200/80 bg-white/95 p-4 sm:p-5", level > 0 && "mt-3")}>
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span className="text-sm font-semibold text-slate-900">{item.author?.name || "Anonymous"}</span>
              {isArticleAuthor ? (
                <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">Author</span>
              ) : null}
              {item.author?.batch ? (
                <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-medium text-blue-700">{item.author.batch}</span>
              ) : null}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
              ...
              {item.canReply ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 font-medium text-slate-600 hover:bg-slate-50"
                >
                  <Reply size={12} />
                  Reply
                </button>
              ...
              {hasReplies ? (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 font-medium text-blue-700 hover:bg-blue-100"
                >
            ...
            {(repliesOpen || hasReplies) && item.canReply ? (
              <div className="mt-3 flex items-center gap-2">
                <input
                  ...
                  className="h-9 w-full rounded-full border border-slate-200 px-3 text-sm outline-none ring-blue-500 transition focus:border-blue-300 focus:ring-2"
                />
                <button
                  type="button"
                  onClick={() => submitReply(item)}
                  disabled={postingReplyId === item.id || !(replyDrafts[item.id] || "").trim()}
                  className="inline-flex h-9 items-center gap-1 rounded-full bg-blue-600 px-3 text-xs font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                >
                  <Send size={12} />
                  Send
                </button>
  ...
  return (
    <section className="relative mt-10 overflow-hidden rounded-3xl border border-slate-200/80 bg-white/95 p-5 shadow-[0_16px_40px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:p-7">
      <div className="pointer-events-none absolute -left-16 -top-16 h-52 w-52 rounded-full bg-blue-200/25 blur-3xl" />
      <div className="pointer-events-none absolute -right-16 -bottom-16 h-52 w-52 rounded-full bg-cyan-200/25 blur-3xl" />

      <div className="relative mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
      ...
      <div className="relative rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
        <div className="mb-2 flex flex-wrap items-center gap-2">
          <span className="text-xs font-medium text-slate-500">Tag as:</span>
        ...
        <textarea
          ...
          className="min-h-[90px] w-full resize-y rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-700 outline-none ring-blue-500 transition focus:border-blue-300 focus:ring-2"
        />
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="rounded-full bg-blue-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300"
          >
            Post
          </button>
        </div>
      ...
      <div className="relative mt-4 flex flex-wrap items-center gap-2 border-b border-slate-200 pb-4">
        {[
          { key: "all", label: "All" },
