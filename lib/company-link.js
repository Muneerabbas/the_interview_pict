...
/**
 * Placement names that cannot be reached by slug or squash, mapped by hand.
 * Every entry is a confirmed identity, not a guess:
 *  - the report abbreviates ("TCS", "OFSS", "LTI") where the directory spells
 *    the company out;
 *  - "L & T Infotech" / "L&T Infotech" / "LTI" are three spellings of one
 *    employer, now LTIMindtree;
 *  - "Agiliad" is ambiguous on its own because the directory holds both
 *    "Agiliad Technologies" and "Agiliad Technology" (a duplicate worth
 *    cleaning up), so the intended target is named explicitly here.
 */
const ALIASES = new Map([
  ["tcs", "tata-consultancy-services-tcs"],
  ["lti", "larsen-toubro-infotech-lti"],
  ["l-t-infotech", "larsen-toubro-infotech-lti"],
  ["lt-infotech", "larsen-toubro-infotech-lti"],
  ["principal-global", "principal-global-services"],
  ["quantiphi", "quantiphi-analytics"],
  ["agiliad", "agiliad-technologies"],
  ["sapient", "publicis-sapient"],
  ["musigma", "mu-sigma"],
  ["ofss", "oracle-financial-services-software"],
  ["hashedin", "hashedin-by-deloitte"],
  ["varian-medical-sys", "varian-medical-systems"],
  ["einfochip", "einfochips"],
  ["reflexis", "reflexis-systems"],
]);
...
export function resolveCompanySlug(name, index) {
  ...
  // Hand-mapped identities win over the fuzzy key, but still only link to a
...
/** Exposed for the coverage script, so an alias is not reported as missing. */
