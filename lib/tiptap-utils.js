...
/**
 * Handles image upload with compression and Cloudinary storage
 * @param file The file to upload
 * @param onProgress Optional callback for tracking upload progress
 * @param abortSignal Optional AbortSignal for cancelling the upload
 * @returns Promise resolving to the Cloudinary URL of the uploaded image
 */
export const handleImageUpload = async (file, onProgress, abortSignal) => {
  ...
  // Dynamically import browser-image-compression (client-only)
  const imageCompression = (await import("browser-image-compression")).default

  // Compress for size while preserving natural aspect ratio and dimensions.
  ...
  const sigResp = await fetch("/api/upload", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({}), // Requesting a fresh signature
    signal: abortSignal,
  })
...
/**
 * Extracts all Cloudinary image URLs from an HTML string.
 * @param html - HTML content from the editor
 * @returns Set of Cloudinary URLs found in the content
 */
export function extractCloudinaryUrls(html) {
  if (!html) return new Set()
  const matches = html.matchAll(/https:\/\/res\.cloudinary\.com\/[^\s"'<>]+/g)
  return new Set([...matches].map((m) => m[0]))
}

/**
 * Parses the Cloudinary public_id from a Cloudinary image URL.
 * Handles URLs with and without version segments.
 * e.g. https://res.cloudinary.com/cloud/image/upload/v123/folder/name.jpg → folder/name
 * @param url - Full Cloudinary URL
 * @returns public_id string, or null if not parseable
 */
export function cloudinaryUrlToPublicId(url) {
  try {
    // Strip everything before and including /upload/
    const afterUpload = url.split("/upload/")[1]
    if (!afterUpload) return null

    // Remove optional version segment (v followed by digits)
 ...
 * Extracts the public_id from the URL and calls /api/delete-image.
 * @param url - Full Cloudinary URL of the image to delete
 */
export async function deleteCloudinaryImage(url) {
  const publicId = cloudinaryUrlToPublicId(url)
  if (!publicId) return
